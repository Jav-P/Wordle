...
public class VistaController {
    ...
    public void unir () {
		...
		if(tamaÒo(a))comparar(a);//hay que buscar la palabra en la base de datos
			
	}
	public static Boolean tamaÒo (String palabra) {
		if(palabra.length()!=5)return false;
		//Sigue la verificaciÛn si es real la palabra
		return true;
	}	
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 0; j < 5; j++) {
				if(palabra.substring(i, i+1).equalsIgnoreCase(objetivo.substring(j, j+1))) {
					if(i==j) {
					System.out.println("La letra "+palabra.substring(i, i+1)+" es: llamar funciÛn verde");
					aux=true;
					}else {
					System.out.println("La letra "+palabra.substring(i, i+1)+" es: funciÛn Amarillo");
					aux=true;
					}
				}			
			}
			if(aux.equals(false))System.out.println("La letra "+palabra.substring(i, i+1)+" es: funciÛn Gris");
		}			
	}
