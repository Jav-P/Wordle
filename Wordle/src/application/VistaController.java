		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 0; j < 5; j++) {
				if(palabra.substring(i, i+1).equalsIgnoreCase(objetivo.substring(j, j+1))) {
					if(i==j) {
					System.out.println("La letra "+palabra.substring(i, i+1)+" es: llamar funci�n verde");
					aux=true;
					}else {
					System.out.println("La letra "+palabra.substring(i, i+1)+" es: funci�n Amarillo");
					aux=true;
					}
				}			
			}
			if(aux.equals(false))System.out.println("La letra "+palabra.substring(i, i+1)+" es: funci�n Gris");
		}			
	}
