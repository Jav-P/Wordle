...
public class VistaController {
	...
	@FXML
	private TextField victorias;
	private TextField derrotas;
	...
	TextField matriz[][] = { { textAA, textAB, textAC, textAD, textAE }, { textBA, textBB, textBC, textBD, textBE },
			{ textCA, textCB, textCC, textCD, textCE }, { textDA, textDB, textDC, textDD, textDE },
			{ textEA, textEB, textEC, textED, textEE }, { textFA, textFB, textFC, textFD, textFE } };
	...
	public void unir() {
		...
		if (tamaÒo(a))
			comparar(a);// hay que buscar la palabra en la base de datos

	}

	public static Boolean tamaÒo(String palabra) {
		if (palabra.length() != 5)
			return false;
		// Sigue la verificaciÔøΩn si es real la palabra
		return true;
	}
	...
	public static void comparar(String palabra) {
		...
		if (palabra.equals(objetivo))
			System.out.println("Felicitaciones, adivino la palabra");// Termina el juego
		for (int i = 0; i < 5; i++) {
			aux = false;
			for (int j = 0; j < 5; j++) {
				...
				for (int i = 0; i < 5; i++) {
					...
					for (int j = 0; j < 5; j++) {
						if (matriz[intento][i].getText().equalsIgnoreCase(objetivo.substring(j, j + 1))) {
							if (i == j) {
								System.out.println(
										"La letra " + palabra.substring(i, i + 1) + " es: llamar funcion verde");
